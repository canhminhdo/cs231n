...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            self.params['W' + str(i + 1)] = np.random.normal(0.0, weight_scale, (layers_dims[i], layers_dims[i + 1]))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            x, cache = affine_norm_relu_forward(x,w,b, gamma, beta, bn_params, self.normalization, self.use_dropout, self.dropout_param)
            caches.append(cache)
        # affine
        ...
        # calculate loss based on softmax
        loss, softmax_grad = softmax_loss(scores, y)
        for i in range(self.num_layers):
            w = self.params['W' + str(i + 1)]
            loss += 0.5 * self.reg * np.sum(w * w)  # add regularization L2
